...
static bool flipp_pomodoro_app_custom_event_callback(void *ctx, uint32_t event)
{
    ...
    case FlippPomodoroAppCustomEventStageComplete:
        ...
        bool is_break_stage = (next_stage == FlippPomodoroStageRest) || (next_stage == FlippPomodoroStageLongBreak);
        FlippPomodoroSettings settings;
        flipp_pomodoro_settings_load(&settings);
        // Keep flash mode completely silent when entering breaks.
        bool skip_beep = (settings.buzz_mode == FlippPomodoroBuzzFlash) && is_break_stage;
